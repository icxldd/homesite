...
import * as daoService from '../../services';
...
export class LoginComponent implements OnInit {
    ...
    public constructor(
        ...
        private homeSrc:daoService.HomeService,
    ...
    public login(): void {
        this.identitySrv.login(this.form.value)
            ...
            //     localStorage.setItem('access_token', res.access_token);
            // }))
            // .pipe(switchMap((res) => this.identitySrv.getProfile()))
            .subscribe(res => {
                localStorage.setItem('access_token', res.access_token);
                this.homeSrc.GetHome().subscribe(x=>{
                    console.log(x);
                })
